...
class Aggregator : gst.element.Element
{
  ...
  */
  ...
  /**
    ...
    ## Parameters
    $(LIST
      * $(B segment)       The #GstSegment the next output buffer is part of
      * $(B pts)       The presentation timestamp of the next output buffer
      * $(B dts)       The decoding timestamp of the next output buffer
      * $(B duration)       The duration of the next output buffer
      * $(B info)       a #GstStructure containing additional information
      * $(B aggregator) the instance the signal is connected to
    )
  */
  alias SamplesSelectedCallbackDlg = void delegate(gst.segment.Segment segment, ulong pts, ulong dts, ulong duration, gst.structure.Structure info, gstbase.aggregator.Aggregator aggregator);

  /** ditto */
  alias SamplesSelectedCallbackFunc = void function(gst.segment.Segment segment, ulong pts, ulong dts, ulong duration, gst.structure.Structure info, gstbase.aggregator.Aggregator aggregator);
  ...
  */
  ...
  if (is(T : SamplesSelectedCallbackDlg) || is(T : SamplesSelectedCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto aggregator = getVal!(gstbase.aggregator.Aggregator)(_paramVals);
      auto segment = getVal!(gst.segment.Segment)(&_paramVals[1]);
      auto pts = getVal!(ulong)(&_paramVals[2]);
      auto dts = getVal!(ulong)(&_paramVals[3]);
      auto duration = getVal!(ulong)(&_paramVals[4]);
      auto info = getVal!(gst.structure.Structure)(&_paramVals[5]);
      _dClosure.dlg(segment, pts, dts, duration, info, aggregator);
    }
