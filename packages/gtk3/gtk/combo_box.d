...
class ComboBox : gtk.bin.Bin, gtk.cell_editable.CellEditable, gtk.cell_layout.CellLayout
{
   ...
   * `gtk_tree_path_get_indices $(LPAREN)path$(RPAREN)[0]`, where
   ...
   * property of combo_box $(LPAREN)see [gtk.combo_box.ComboBox.setIdColumn]$(RPAREN).
   ...
   * always sensitive $(LPAREN)%GTK_SENSITIVITY_ON$(RPAREN), never sensitive $(LPAREN)%GTK_SENSITIVITY_OFF$(RPAREN)
   * or only if there is at least one item to display $(LPAREN)%GTK_SENSITIVITY_AUTO$(RPAREN).
   * Params:
   ...
   * model $(LPAREN)if applicable$(RPAREN). If model is %NULL, then it will unset the model.
  ...
  /**
   * For combo boxes that are created with an entry $(LPAREN)See GtkComboBox:has-entry$(RPAREN).
