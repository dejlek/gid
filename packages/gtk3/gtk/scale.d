...
class Scale : gtk.range.Range
{
  ...
  /**
   * If #GtkScale:has-origin is set to %TRUE $(LPAREN)the default$(RPAREN), the scale will
   * highlight the part of the trough between the origin $(LPAREN)bottom or left side$(RPAREN)
   ...
   * |[<!-- language\="C" -->
   * static gchar*
   * format_value_callback $(LPAREN)GtkScale *scale,
   * gdouble   value$(RPAREN)
   * {
   * return g_strdup_printf $(LPAREN)"-->\%0.*g<--",
   * gtk_scale_get_digits $(LPAREN)scale$(RPAREN), value$(RPAREN);
   * }
   * ]|
   * Params
   *   value = the value to format
   *   scale = the instance the signal is connected to
   * Returns: allocated string representing value
   */
  alias FormatValueCallbackDlg = string delegate(double value, gtk.scale.Scale scale);
  alias FormatValueCallbackFunc = string function(double value, gtk.scale.Scale scale);
  ...
  if (is(T : FormatValueCallbackDlg) || is(T : FormatValueCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto scale = getVal!(gtk.scale.Scale)(_paramVals);
      auto value = getVal!(double)(&_paramVals[1]);
      auto _retval = _dClosure.dlg(value, scale);
