...
class Clipboard : gobject.object.ObjectG
{
   ...
   * $(LPAREN)%GDK_NONE is supported as a synonym for GDK_SELECTION_CLIPBOARD
   * for backwards compatibility reasons.$(RPAREN)
   * The currently-selected object or text should be provided on the clipboard
   * identified by #GDK_SELECTION_PRIMARY. Cut/copy/paste menu items
   * conceptually copy the contents of the #GDK_SELECTION_PRIMARY clipboard
   * to the default clipboard, i.e. they copy the selection to what the
   * user sees as the clipboard.
   * $(LPAREN)Passing #GDK_NONE is the same as using `gdk_atom_intern
   * $(LPAREN)"CLIPBOARD", FALSE$(RPAREN)`.
   * See the
   * [FreeDesktop Clipboard Specification](http://www.freedesktop.org/Standards/clipboards-spec)
   * for a detailed discussion of the “CLIPBOARD” vs. “PRIMARY”
   * selections under the X window system. On Win32 the
   * #GDK_SELECTION_PRIMARY clipboard is essentially ignored.$(RPAREN)
   * It’s possible to have arbitrary named clipboards; if you do invent
   * new clipboards, you should prefix the selection name with an
   * underscore $(LPAREN)because the ICCCM requires that nonstandard atoms are
   * underscore-prefixed$(RPAREN), and namespace it as well. For example,
   ...
   *     $(LPAREN)or the retrieval fails$(RPAREN). If the retrieval fails the length field of
   ...
   *     or the retrieval fails. $(LPAREN)It will always be called one way or the other.$(RPAREN)
   */
   ...
   * application exits or when gtk_clipboard_store $(LPAREN)$(RPAREN) is called.
   * This value is reset when the clipboard owner changes.
   * Where the clipboard data is stored is platform dependent,
   * see gdk_display_store_clipboard $(LPAREN)$(RPAREN) for more information.
   * Params:
   ...
   *     the length will be determined with strlen$(LPAREN)$(RPAREN).
   */
   ...
   *   %NULL if retrieving the selection data failed. $(LPAREN)This could
   *   happen for various reasons, in particular if the clipboard
   *   was empty or if the contents of the clipboard could not be
   *   converted into an image.$(RPAREN)
   */
   ...
   *   the selection data failed. $(LPAREN)This could happen
   *   for various reasons, in particular if the
   *   clipboard was empty or if the contents of the
   *   clipboard could not be converted into text form.$(RPAREN)
   */
  string waitForText()
  {
    char* _cretval;
   ...
   *   selection data failed. $(LPAREN)This could happen for various reasons,
   *   in particular if the clipboard was empty or if the contents of
   *   the clipboard could not be converted into URI form.$(RPAREN)
   */
  string[] waitForUris()
  {
    char** _cretval;
   ...
   * gtk_clipboard_wait_is_text_available $(LPAREN)$(RPAREN) instead.
   * Params:
