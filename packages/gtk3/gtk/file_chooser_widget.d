...
class FileChooserWidget : gtk.box.Box, gtk.file_chooser.FileChooser
{
   ...
   * of "" $(LPAREN)the empty string$(RPAREN).  It is also bound to `/` with a path string of
   * "`/`" $(LPAREN)a slash$(RPAREN):  this lets you type `/` and immediately type a path name.
   * On Unix systems, this is bound to `~` $(LPAREN)tilde$(RPAREN) with a path string of "~"
