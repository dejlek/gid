...
class PlacesSidebar : gtk.scrolled_window.ScrolledWindow
{
   ...
   *   |[<!-- language\="C" -->
   *   g_slist_free_full $(LPAREN)list, $(LPAREN)GDestroyNotify$(RPAREN) g_object_unref$(RPAREN);
   *   ]|
   */
   ...
   * application can open new locations, so that the sidebar can display $(LPAREN)or not$(RPAREN)
   ...
   *   placesSidebar = the instance the signal is connected to
   ...
   * dismisses the popup. The popup is re-created $(LPAREN)and thus, this signal is
   * emitted$(RPAREN) every time the user activates the contextual menu.
