...
class Application : gio.application.Application
{
   ...
   * currently inhibited $(LPAREN)possibly by another application$(RPAREN).
   * Note that this information may not be available $(LPAREN)for example
   * when the application is running in a sandbox$(RPAREN).
   * Params:
   ...
   * also be created by the desktop environment $(LPAREN)GNOME, for example, uses
   * a menu with only a "Quit" item in it$(RPAREN).
   * The value returned by this function never changes.  Once it returns a
   * particular value, it is guaranteed to always return the same value.
   * You may only call this function after the application has been
   * registered and after the base startup handler has run.  You're most
   * likely to want to use this from your own startup handler.  It may
   * also make sense to consult this function while constructing UI $(LPAREN)in
   * activate, open or an action activation handler$(RPAREN) in order to determine
