...
class SizeGroup : gobject.object.ObjectG, gtk.buildable.Buildable
{
  ...
  {
    bool _retval;
    _retval = gtk_size_group_get_ignore_hidden(cast(GtkSizeGroup*)cPtr);
    return _retval;
  }
  ...
  {
    gtk_size_group_set_ignore_hidden(cast(GtkSizeGroup*)cPtr, ignoreHidden);
  }

  /**
   * Sets the #GtkSizeGroupMode of the size group. The mode of the size
   * group determines whether the widgets in the size group should
   * all have the same horizontal requisition $(LPAREN)%GTK_SIZE_GROUP_HORIZONTAL$(RPAREN)
   * all have the same vertical requisition $(LPAREN)%GTK_SIZE_GROUP_VERTICAL$(RPAREN),
   * or should all have the same requisition in both directions
   * $(LPAREN)%GTK_SIZE_GROUP_BOTH$(RPAREN).
   * Params:
