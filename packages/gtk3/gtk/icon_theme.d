...
class IconTheme : gobject.object.ObjectG
{
   ...
   *   size = the size in pixels at which to register the icon $(LPAREN)different
   *     images can be registered for the same icon name at different sizes.$(RPAREN)
   ...
   * [gtk.icon_info.IconInfo.loadIcon]. []$(LPAREN)gtk.icon_theme.IconTheme.loadIcon
   * combines these two steps if all you need is the pixbuf.$(RPAREN)
   ...
   * current theme $(LPAREN)for instance, to use when presenting
   * a list of themes to the user.$(RPAREN)
   ...
   * icon. The icon can then be rendered into a pixbuf using
   * [gtk.icon_info.IconInfo.loadIcon]. []$(LPAREN)gtk.icon_theme.IconTheme.loadIcon combines
   * these two steps if all you need is the pixbuf.$(RPAREN)
   * Params:
   *   iconName = the name of the icon to lookup
   *   size = desired icon size
   *   scale = the desired scale
