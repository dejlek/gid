...
class Style : gobject.object.ObjectG
{
   ...
   *   state = a state
   *   size = the size to render the icon at $(LPAREN)#GtkIconSize$(RPAREN). A size of
   *     `$(LPAREN)GtkIconSize$(RPAREN)-1` means render at the size of the source and
   *     don’t scale.
   *   widget = the widget
   ...
   *   style = the instance the signal is connected to
   */
