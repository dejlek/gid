...
class Gesture : gtk.event_controller.EventController
{
   ...
   * handler$(LPAREN)s$(RPAREN) returned #TRUE.
   * Note: These conditions may also happen when an extra touch $(LPAREN)eg. a third touch
   * on a 2-touches gesture$(RPAREN) is lifted, in that situation sequence won't pertain
   ...
   * on gesture $(LPAREN)manually, due to grabs...$(RPAREN), or the individual sequence
   * was claimed by parent widgets' controllers $(LPAREN)see [gtk.gesture.Gesture.setSequenceState]$(RPAREN).
   ...
   * sequences as something to be handled $(LPAREN)the #GtkGesture::check handler returned
   * %FALSE$(RPAREN), or the number of touch sequences became higher or lower than
   * #GtkGesture:n-points.
   * Note: sequence might not pertain to the group of sequences that were
   * previously triggering recognition on gesture $(LPAREN)ie. a just pressed touch
   * sequence that exceeds #GtkGesture:n-points$(RPAREN). This situation may be detected
