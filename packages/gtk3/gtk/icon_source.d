...
class IconSource : gobject.boxed.Boxed
{
  ...
  /**
   * Creates a new #GtkIconSource. A #GtkIconSource contains a #GdkPixbuf $(LPAREN)or
   * image filename$(RPAREN) that serves as the base image for one or more of the
   ...
   * on toolbar mode $(LPAREN)small/large icons$(RPAREN). So a single icon set would
   ...
   * for the GtkIconSource passed to the #GtkStyle render_icon$(LPAREN)$(RPAREN)
   ...
   * Returns: icon size $(LPAREN)#GtkIconSize$(RPAREN) this source matches.
   ...
   * #GtkIconSet prefers non-wildcarded sources $(LPAREN)exact matches$(RPAREN) over
   ...
   * in addition to calling this function.
   * Params:
   *   size = icon size $(LPAREN)#GtkIconSize$(RPAREN) this source applies to
