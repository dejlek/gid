...
class IconView : gtk.container.Container, gtk.cell_layout.CellLayout, gtk.scrollable.Scrollable
{
   ...
   *     responsible for the cell at $(LPAREN)x, y$(RPAREN), or %NULL
   ...
   * Returns:
   */
  alias MoveCursorCallbackDlg = bool delegate(gtk.types.MovementStep step, int count, gtk.icon_view.IconView iconView);
  alias MoveCursorCallbackFunc = bool function(gtk.types.MovementStep step, int count, gtk.icon_view.IconView iconView);
  ...
  if (is(T : MoveCursorCallbackDlg) || is(T : MoveCursorCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      _retval = _dClosure.dlg(step, count, iconView);
      setVal!bool(_returnValue, _retval);
    }
