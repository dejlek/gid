...
class IconSet : gobject.boxed.Boxed
{
   ...
   * and it might change size depending on toolbar mode $(LPAREN)small/large
   * icons$(RPAREN). So a single icon set would contain all those variants of
   ...
   *     for array of sizes $(LPAREN)#GtkIconSize$(RPAREN)
   ...
   * $(LPAREN)perhaps because an image file fails to load$(RPAREN), a default "missing
   * image" icon will be returned instead.
   * Params:
   *   style = a #GtkStyle associated with widget, or %NULL
   *   direction = text direction
   *   state = widget state
   *   size = icon size $(LPAREN)#GtkIconSize$(RPAREN). A size of `$(LPAREN)GtkIconSize$(RPAREN)-1`
