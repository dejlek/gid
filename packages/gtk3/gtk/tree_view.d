...
class TreeView : gtk.container.Container, gtk.scrollable.Scrollable
{
  ...
  /**
   * Converts bin_window coordinates $(LPAREN)see [gtk.tree_view.TreeView.getBinWindow]$(RPAREN)
   ...
   * $(LPAREN)see [gtk.tree_view.TreeView.getBinWindow]$(RPAREN).
   * Params:
  ...
  void enableModelDragDest(gtk.target_entry.TargetEntry[] targets, gdk.types.DragAction actions)
  {
    ...
    gtk_tree_view_enable_model_drag_dest(cast(GtkTreeView*)cPtr, _targets, _nTargets, actions);
  }
  ...
  void enableModelDragSource(gdk.types.ModifierType startButtonMask, gtk.target_entry.TargetEntry[] targets, gdk.types.DragAction actions)
  {
    ...
    foreach (obj; targets)
      _tmptargets ~= *cast(GtkTargetEntry*)obj.cPtr;
    const(GtkTargetEntry)* _targets = _tmptargets.ptr;
    gtk_tree_view_enable_model_drag_source(cast(GtkTreeView*)cPtr, startButtonMask, _targets, _nTargets, actions);
  }
   ...
   * $(LPAREN)please see [gtk.tree_view.TreeView.getBinWindow]$(RPAREN).
   * That is, x and y are relative to an events coordinates. x and y must
   * come from an event on the tree_view only where `event->window \=\=
   * gtk_tree_view_get_bin_window $(LPAREN)$(RPAREN)`. It is primarily for
  ...
  {
    bool _retval;
    _retval = gtk_tree_view_get_rules_hint(cast(GtkTreeView*)cPtr);
    return _retval;
  }
   ...
   * where `event->window \=\= gtk_tree_view_get_bin_window $(LPAREN)$(RPAREN)`.
   * For converting widget coordinates $(LPAREN)eg. the ones you get from
   * GtkWidget::query-tooltip$(RPAREN), please see
   * [gtk.tree_view.TreeView.convertWidgetToBinWindowCoords].
  ...
  void setDestroyCountFunc(gtk.types.TreeDestroyCountFunc func)
  {
    ...
    auto _func = func ? freezeDelegate(cast(void*)&func) : null;
    GDestroyNotify _funcDestroyCB = func ? &thawDelegate : null;
    gtk_tree_view_set_destroy_count_func(cast(GtkTreeView*)cPtr, _funcCB, _func, _funcDestroyCB);
  }
   ...
   * from a functional standpoint $(LPAREN)since it has lots of columns,
   * generally$(RPAREN).
   * Params:
  ...
  {
    gtk_tree_view_set_rules_hint(cast(GtkTreeView*)cPtr, setting);
  }
  ...
  void setSearchPositionFunc(gtk.types.TreeViewSearchPositionFunc func)
  {
    ...
    auto _func = func ? freezeDelegate(cast(void*)&func) : null;
    GDestroyNotify _funcDestroyCB = func ? &thawDelegate : null;
    gtk_tree_view_set_search_position_func(cast(GtkTreeView*)cPtr, _funcCB, _func, _funcDestroyCB);
  }
   ...
   */
  alias MoveCursorCallbackDlg = bool delegate(gtk.types.MovementStep step, int direction, gtk.tree_view.TreeView treeView);
  alias MoveCursorCallbackFunc = bool function(gtk.types.MovementStep step, int direction, gtk.tree_view.TreeView treeView);
  ...
  if (is(T : MoveCursorCallbackDlg) || is(T : MoveCursorCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      _retval = _dClosure.dlg(step, direction, treeView);
      setVal!bool(_returnValue, _retval);
    }
