...
/**
 * Controls how a widget deals with extra space in a single $(LPAREN)x or y$(RPAREN)
 * dimension.
...
/**
 * Denotes the expansion properties that a widget will have when it $(LPAREN)or its
 * parent$(RPAREN) is resized.
 */
...
enum GtkButtonBoxStyle
{
   ...
   * $(LPAREN)on the left for a HBox, or the top for a VBox$(RPAREN).
   */
  Start = 3,

  /**
   * Buttons are grouped towards the end of the box,
   * $(LPAREN)on the right for a HBox, or the bottom for a VBox$(RPAREN).
   */
  End = 4,
   ...
   * setting spacing to 0 $(LPAREN)same as calling [gtk.box.Box.setHomogeneous] and
   * [gtk.box.Box.setSpacing] manually$(RPAREN). Since 3.12.
   */
  Expand = 6,
}
...
enum GtkCssSectionType
{
   ...
   * Animations]$(LPAREN)http://dev.w3.org/csswg/css3-animations/#keyframes$(RPAREN) for details. Since 3.6
   */
  Keyframes = 8,
}
...
enum GtkFileChooserError
{
  ...
  /**
   * Indicates an incomplete hostname $(LPAREN)e.g. "http://foo" without a slash after that$(RPAREN).
   */
...
enum GtkIconSize
{
  ...
  /**
   * Size appropriate for menus $(LPAREN)16px$(RPAREN).
   */
  Menu = 1,

  /**
   * Size appropriate for small toolbars $(LPAREN)16px$(RPAREN).
   */
  SmallToolbar = 2,

  /**
   * Size appropriate for large toolbars $(LPAREN)24px$(RPAREN)
   */
  LargeToolbar = 3,

  /**
   * Size appropriate for buttons $(LPAREN)16px$(RPAREN)
   */
  Button = 4,

  /**
   * Size appropriate for drag and drop $(LPAREN)32px$(RPAREN)
   */
  Dnd = 5,

  /**
   * Size appropriate for dialogs $(LPAREN)48px$(RPAREN)
   */
  Dialog = 6,
}
 ...
 * images, you can request any storage type $(LPAREN)call any of the "get"
...
/**
 * Used for justifying the text inside a #GtkLabel widget. $(LPAREN)See also
 * #GtkAlignment$(RPAREN).
 */
...
/**
 * Represents the packing location #GtkBox children. $(LPAREN)See: #GtkVBox,
 * #GtkHBox, and #GtkButtonBox$(RPAREN).
 */
enum GtkPackType
{
  /**
 ...
 * various shadow types: Either their is no visible shadow $(LPAREN)@GTK_SHADOW_NONE$(RPAREN),
 * or there is $(LPAREN)any other value$(RPAREN).
 */
enum GtkShadowType
{
  /**
...
enum GtkTextViewLayer
{
  ...
  /**
   * The layer rendered below the text $(LPAREN)but above the background$(RPAREN).  Since: 3.20
   */
  BelowText = 2,
...
enum GtkWindowPosition
{
   ...
   * parent $(LPAREN)see [gtk.window.Window.setTransientFor]$(RPAREN).
   */
  CenterOnParent = 4,
}
 ...
 * are managed by the window manager and have a frame by default $(LPAREN)call
 * [gtk.window.Window.setDecorated] to toggle the frame$(RPAREN).  Windows with type
 * #GTK_WINDOW_POPUP are ignored by the window manager; window manager
 * keybindings won’t work on them, the window manager won’t decorate the
 * window with a frame, many GTK+ features that rely on the window
 * manager will not work $(LPAREN)e.g. resize grips and
 * maximization/minimization$(RPAREN). #GTK_WINDOW_POPUP is used to implement
 ...
 * |[<!-- language\="C" -->
 * GdkPixbuf *example_logo \= gdk_pixbuf_new_from_file $(LPAREN)"./logo.png", NULL$(RPAREN);
 ...
 * typically attached to a toplevel #GtkWindow $(LPAREN)with
 * [gtk.window.Window.addAccelGroup]$(RPAREN). Usually you won’t need to create a
 ...
 * - a name $(LPAREN)not translated, for path lookup$(RPAREN)
 * - a label $(LPAREN)translated, for display$(RPAREN)
 * - an accelerator
 * - whether label indicates a stock id
 * - a tooltip $(LPAREN)optional, translated$(RPAREN)
 * - a toolbar label $(LPAREN)optional, shorter than label$(RPAREN)
 * The action will also have some state information:
 * - visible $(LPAREN)shown/hidden$(RPAREN)
 * - sensitive $(LPAREN)enabled/disabled$(RPAREN)
 ...
 * and #GtkRange $(LPAREN)which is a base class for #GtkScrollbar and #GtkScale$(RPAREN).
 ...
 * The values can range from 0 $(LPAREN)meaning the child doesn’t expand at all$(RPAREN) to
 * 1 $(LPAREN)meaning the child expands to fill all of the available space$(RPAREN).
 * The align settings are used to place the child widget within the available
 * area. The values range from 0 $(LPAREN)top or left$(RPAREN) to 1 $(LPAREN)bottom or right$(RPAREN).
 ...
 * #GtkApplicationWindow and sets up keyboard accelerators $(LPAREN)Control-F1
 * and Control-?$(RPAREN) to open it. To create a menu item that displays the
 ...
 * of the users session $(LPAREN)if you set the #GtkApplication:register-session
 ...
 * be held when activating actions locally $(LPAREN)if GDK threads are enabled$(RPAREN).
 ...
 * on Windows neither will be. gnome-shell $(LPAREN)starting with version 3.4$(RPAREN)
 ...
 * "</interface>",
 * -1$(RPAREN);
 * GMenuModel *menubar \= G_MENU_MODEL $(LPAREN)gtk_builder_get_object $(LPAREN)builder,
 * "menubar"$(RPAREN)$(RPAREN);
 ...
 * one of the four cardinal directions $(LPAREN)up, down, left, or right$(RPAREN).  The
...
struct GtkBuildableIface
{
   ...
   * $(LPAREN)i.e. setting the #GtkWidget:visible property$(RPAREN) until the whole interface
   * is created.
   */
 ...
 * $(LPAREN)multiple$(RPAREN) calls to [gtk.builder.Builder.addFromFile],
 ...
 * larger hierarchy constructed by the builder $(LPAREN)in which case you should
 * not have to worry about their lifecycle$(RPAREN), or without a parent, in which
 ...
 * child objects $(LPAREN)most often widgets inside a container, but also e.g.
 * actions in an action group, or columns in a tree model$(RPAREN). A `<child>`
 ...
 * necessary, it is possible to specify the name of the get_type$(LPAREN)$(RPAREN) function
 ...
 * with `___` $(LPAREN)3 underscores$(RPAREN) for its own purposes.
 ...
 * as %FALSE$(RPAREN), enumerations $(LPAREN)can be specified by their name, nick or
 * integer value$(RPAREN), flags $(LPAREN)can be specified by their name, nick, integer
 * value, optionally combined with “|”, e.g. “GTK_VISIBLE|GTK_REALIZED”$(RPAREN)
 * and colors $(LPAREN)in a format understood by [gdk.rgba.RGBA.parse]$(RPAREN).
 ...
 * properties on them or to add further children $(LPAREN)e.g. the @vbox of
 * a #GtkDialog$(RPAREN). This can be achieved by setting the “internal-child”
 ...
 * $(LPAREN)also referred to as "layouting widgets"$(RPAREN) to interface with an
 ...
 * #GtkCellArea which created it $(LPAREN)see [gtk.cell_area.CellArea.createContext]$(RPAREN).
 ...
 * idle or timeout source $(LPAREN)see [glib.global.timeoutAdd]$(RPAREN) and when the widget
 * is requested its actual width in #GtkWidgetClass.get_preferred_width$(LPAREN)$(RPAREN)
 ...
 * gint            *minimum_size,
 * gint            *natural_size$(RPAREN)
 * {
 * Foo        *foo  \= FOO $(LPAREN)widget$(RPAREN);
 * FooPrivate *priv \= foo->priv;
 * foo_ensure_at_least_one_handfull_of_rows_have_been_requested $(LPAREN)foo$(RPAREN);
 ...
 * call to #GtkWidgetClass.get_preferred_height_for_width$(LPAREN)$(RPAREN). Returning
 ...
 * #GtkWidgetClass.draw$(LPAREN)$(RPAREN) time.
 ...
 * #GtkWidgetClass.focus$(LPAREN)$(RPAREN) virtual method. The layouting widget is always
 * responsible for knowing where #GtkTreeModel rows are rendered inside
 * the widget, so at #GtkWidgetClass.focus$(LPAREN)$(RPAREN) time the layouting widget
 * should use the #GtkCellArea methods to navigate focus inside the area
 * and then observe the GtkDirectionType to pass the focus to adjacent
 * rows and areas.
 * A basic example of how the #GtkWidgetClass.focus$(LPAREN)$(RPAREN) virtual method
...
struct GtkCellAreaClass
{
   ...
   * of other sibling cells $(LPAREN)see [gtk.cell_area.CellArea.getFocusFromSibling]$(RPAREN).
   ...
   * #GtkCellArea activates the focus cell $(LPAREN)see [gtk.cell_area.CellArea.getFocusCell]$(RPAREN).
   */
 ...
 * to lay out cells $(LPAREN)all GtkCellLayouts in GTK+ use a GtkCellArea$(RPAREN)
 ...
 * |[<!-- language\="C" -->
 * combo \= g_object_new $(LPAREN)GTK_TYPE_COMBO_BOX, "cell-area", my_cell_area, NULL$(RPAREN);
 * ]|
 ...
 * be properly aligned with eachother $(LPAREN)like the aligned cells in the menus
 ...
 * $(LPAREN)usually a #GtkLabel$(RPAREN). See the section on #GtkToggleButton widgets for
 * more information about toggle/check buttons.
 * The important signal $(LPAREN) #GtkToggleButton::toggled $(RPAREN) is also inherited from
 * #GtkToggleButton.
 * # CSS nodes
 * |[<!-- language\="plain" -->
 * checkbutton
 ...
 * ╰── <child>
 * ]|
 * A GtkCheckButton with indicator $(LPAREN)see [gtk.toggle_button.ToggleButton.setMode]$(RPAREN) has a
 ...
 * #GdkAtom. $(LPAREN)Conversion to and from strings can be done with
 * [gdk.atom.Atom.intern] and [gdk.atom.Atom.name].$(RPAREN) The default clipboard
 ...
 * of the clipboard, you can either supply the data directly $(LPAREN)via
 * functions like [gtk.clipboard.Clipboard.setText]$(RPAREN), or you can supply a
 * callback to be called at a later time when the data is needed $(LPAREN)via
 * [gtk.clipboard.Clipboard.setWithData] or [gtk.clipboard.Clipboard.setWithOwner].$(RPAREN)
 ...
 * $(LPAREN)As an example, for the PRIMARY clipboard, when an entry widget
 * provides the clipboard’s contents the contents are simply the
 * text within the selected region.$(RPAREN) If the contents change, the
 ...
 * callback that will be called when the contents are received $(LPAREN)or
 * when the request fails.$(RPAREN) If you don’t want to deal with providing
 ...
 * and converting the results into the UTF-8 encoding. $(LPAREN)The standard
 * form for representing strings in GTK+.$(RPAREN)
 */
struct GtkClipboard;
 ...
 * For implementations of #GtkContainer the virtual method #GtkContainerClass.forall$(LPAREN)$(RPAREN)
 * is always required, since it's used for drawing and other internal operations
 * on the children.
 * If the #GtkContainer implementation expect to have non internal children
 * it's needed to implement both #GtkContainerClass.add$(LPAREN)$(RPAREN) and #GtkContainerClass.remove$(LPAREN)$(RPAREN).
 ...
 * in the #GtkWidgetClass.destroy$(LPAREN)$(RPAREN) implementation.
 * See more about implementing custom widgets at https://wiki.gnome.org/HowDoI/CustomWidgets
 * # Height for width geometry management
 * GTK+ uses a height-for-width $(LPAREN)and width-for-height$(RPAREN) geometry management system.
 * Height-for-width means that a widget can change how much vertical space it needs,
 * depending on the amount of horizontal space that it is given $(LPAREN)and similar for
 * width-for-height$(RPAREN).
 ...
 * when implementing height-for-width $(LPAREN)or width-for-height$(RPAREN) containers.
 ...
 * |[<!-- language\="C" -->
 * static void
 * foo_container_get_preferred_height $(LPAREN)GtkWidget *widget,
 * gint *min_height,
 ...
 * vertically $(LPAREN)or whatever is appropriate for this container$(RPAREN)
 * ...
 * }
 * }
 * ]|
 ...
 * |[<!-- language\="C" -->
 * static void
 * foo_container_get_preferred_width_for_height $(LPAREN)GtkWidget *widget,
 * gint for_height,
 * gint *min_width,
 ...
 * would implement the get_preferred_height_for_width$(LPAREN)$(RPAREN) virtual function by first calling
 ...
 * extra space $(LPAREN)if and where appropriate$(RPAREN) for the widget to expand.
 ...
 * #GtkRequestedSize array for any widgets that stack vertically $(LPAREN)for tabular containers this can
 * be generalized into the heights and widths of rows and columns$(RPAREN).
 ...
 * the current theme $(LPAREN)see the #GtkSettings:gtk-theme-name setting$(RPAREN), `DATADIR`
 * is the prefix configured when GTK+ was compiled $(LPAREN)unless overridden by the
 * `GTK_DATA_PREFIX` environment variable$(RPAREN), and `VERSION` is the GTK+ version number.
 ...
 * from user input$(RPAREN), can be created by calling [gtk.window.Window.setModal] on the
 * dialog. Use the GTK_WINDOW$(LPAREN)$(RPAREN) macro to cast the widget returned from
 ...
 * #GtkResponseType enumeration $(LPAREN)these all have values less than zero$(RPAREN). If
 ...
 * void
 * quick_message $(LPAREN)GtkWindow *parent, gchar *message$(RPAREN)
 * {
 ...
 * "response",
 * G_CALLBACK $(LPAREN)gtk_widget_destroy$(RPAREN),
 * dialog$(RPAREN);
 * // Add the label, and show everything we’ve added
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)content_area$(RPAREN), label$(RPAREN);
 * gtk_widget_show_all $(LPAREN)dialog$(RPAREN);
 * }
 * ]|
 ...
 * $(LPAREN)such as #GtkCellLayoutDataFuncs or #GtkCellArea::apply-attributes$(RPAREN)
 ...
 * sandboxed environment without direct filesystem access $(LPAREN)such as Flatpak$(RPAREN),
 * #GtkFileChooserNative may call the proper APIs $(LPAREN)portals$(RPAREN) to let the user
 ...
 * "_Open",
 * "_Cancel"$(RPAREN);
 * res \= gtk_native_dialog_run $(LPAREN)GTK_NATIVE_DIALOG $(LPAREN)native$(RPAREN)$(RPAREN);
 * if $(LPAREN)res \=\= GTK_RESPONSE_ACCEPT$(RPAREN)
 * {
 * char *filename;
 ...
 * [gtk.file_filter.FileFilter.addPattern]$(RPAREN), on their mime type $(LPAREN)with
 * [gtk.file_filter.FileFilter.addMimeType]$(RPAREN), or by a custom filter function
 * $(LPAREN)with [gtk.file_filter.FileFilter.addCustom]$(RPAREN).
 ...
 * [gtk.frame.Frame.setShadowType] with %GTK_SHADOW_NONE to add the “.flat” class or
 ...
 * or subclass #GtkGLArea and override the @GtkGLAreaClass.render$(LPAREN)$(RPAREN) virtual
 * function.
 ...
 * it is suitable to implement single-touch and pointer-based gestures $(LPAREN)using
 * the special %NULL #GdkEventSequence value for these$(RPAREN).
...
/**
 * #GtkGestureSingle is a subclass of #GtkGesture, optimized $(LPAREN)although
 * not restricted$(RPAREN) for dealing with mouse and single-touch gestures. Under
 ...
 * symbolic color references by the colors they refer to $(LPAREN)in a given
 * context$(RPAREN) and constructs a #cairo_pattern_t value.
 ...
 * |[<!-- language\="C" -->
 * void im_module_init$(LPAREN)GTypeModule *module$(RPAREN);
 * ]|
 ...
 * |[<!-- language\="C" -->
 * void im_module_exit$(LPAREN)void$(RPAREN);
 * ]|
 * Here goes any cleanup code your input method might require on module unload.
 * |[<!-- language\="C" -->
 * void im_module_list$(LPAREN)const GtkIMContextInfo ***contexts, int *n_contexts$(RPAREN)
 * {
 * *contexts \= info_list;
 * *n_contexts \= G_N_ELEMENTS $(LPAREN)info_list$(RPAREN);
 * }
 * ]|
 ...
 * |[<!-- language\="C" -->
 * GtkIMContext * im_module_create$(LPAREN)const gchar *context_id$(RPAREN);
 * ]|
 * This function should return a pointer to a newly created instance of the
 * #GtkIMContext subclass identified by @context_id. The context ID is the same
 * as specified in the #GtkIMContextInfo array returned by im_module_list$(LPAREN)$(RPAREN).
...
struct GtkIMContextInfo
{
  ...
  /**
   * Translation domain to be used with dgettext$(LPAREN)$(RPAREN)
   */
  const(char)* domain;

  /**
   * Name of locale directory for use with bindtextdomain$(LPAREN)$(RPAREN)
   */
 ...
 * Compose file$(RPAREN). The syntax of these files is described in the Compose$(LPAREN)5$(RPAREN)
 * manual page.
 * ## Unicode characters
 ...
 * set of variants of a particular icon $(LPAREN)i.e. a #GtkIconSet contains variants for
 * different sizes and widget states$(RPAREN). Icons in an icon factory are named by a
 ...
 * terms of one or more named icons. $(LPAREN)An example of the
 * more than one case is icons that depend on writing direction;
 * %GTK_STOCK_GO_FORWARD uses the two themed icons
 * “gtk-stock-go-forward-ltr” and “gtk-stock-go-forward-rtl”.$(RPAREN)
 ...
 * GdkPixbuf *pixbuf;
 * icon_theme \= gtk_icon_theme_get_default $(LPAREN)$(RPAREN);
 * pixbuf \= gtk_icon_theme_load_icon $(LPAREN)icon_theme,
 ...
 * if $(LPAREN)!pixbuf$(RPAREN)
 * {
 * g_warning $(LPAREN)"Couldn’t load icon: %s", error->message$(RPAREN);
 * g_error_free $(LPAREN)error$(RPAREN);
 * }
 * else
 * {
 ...
 * gtk_widget_set_no_show_all $(LPAREN)widget, TRUE$(RPAREN);
 * message_label \= gtk_label_new $(LPAREN)""$(RPAREN);
 * content_area \= gtk_info_bar_get_content_area $(LPAREN)bar$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)content_area$(RPAREN),
 * message_label$(RPAREN);
 ...
 * gtk_info_bar_set_message_type $(LPAREN)bar,
 * GTK_MESSAGE_ERROR$(RPAREN);
 ...
 * g_signal_connect_swapped $(LPAREN)window, "button_press_event",
 * G_CALLBACK $(LPAREN)my_popup_handler$(RPAREN), menu$(RPAREN);
 * ]|
 * ## Signal handler which displays a popup menu.
 * |[<!-- language\="C" -->
 * static gint
 * my_popup_handler $(LPAREN)GtkWidget *widget, GdkEvent *event$(RPAREN)
 * {
 * GtkMenu *menu;
 * GdkEventButton *event_button;
 * g_return_val_if_fail $(LPAREN)widget !\= NULL, FALSE$(RPAREN);
 * g_return_val_if_fail $(LPAREN)GTK_IS_MENU $(LPAREN)widget$(RPAREN), FALSE$(RPAREN);
 * g_return_val_if_fail $(LPAREN)event !\= NULL, FALSE$(RPAREN);
 * // The "widget" is the menu that was supplied when
 * // g_signal_connect_swapped$(LPAREN)$(RPAREN) was called.
 * menu \= GTK_MENU $(LPAREN)widget$(RPAREN);
 * if $(LPAREN)event->type \=\= GDK_BUTTON_PRESS$(RPAREN)
 * {
 * event_button \= $(LPAREN)GdkEventButton *$(RPAREN) event;
 * if $(LPAREN)event_button->button \=\= GDK_BUTTON_SECONDARY$(RPAREN)
 * {
 * gtk_menu_popup $(LPAREN)menu, NULL, NULL, NULL, NULL,
 * event_button->button, event_button->time$(RPAREN);
 * return TRUE;
 * }
 * }
 * return FALSE;
 ...
 * you can also pass in the %GTK_DIALOG_MODAL flag, [gtk.dialog.Dialog.run] automatically
 ...
 * gtk_dialog_run $(LPAREN)GTK_DIALOG $(LPAREN)dialog$(RPAREN)$(RPAREN);
 * gtk_widget_destroy $(LPAREN)dialog$(RPAREN);
 * ]|
 ...
 * g_signal_connect_swapped $(LPAREN)dialog, "response",
 * G_CALLBACK $(LPAREN)gtk_widget_destroy$(RPAREN),
 * dialog$(RPAREN);
 * ]|
 ...
 * $(LPAREN)i.e. whether it is a plain, check or radio button$(RPAREN) is determined by
 ...
 * ╰── check
 * ]|
 * Iconic model buttons $(LPAREN)see #GtkModelButton:iconic$(RPAREN) change the name of
 ...
 * $(LPAREN)see [gtk.notebook.Notebook.setTabPos]$(RPAREN), whether, if there are too many
 * tabs to fit the notebook should be made bigger or scrolling
 * arrows added $(LPAREN)see [gtk.notebook.Notebook.setScrollable]$(RPAREN), and whether there
 * will be a popup menu allowing the users to switch pages.
 * $(LPAREN)see [gtk.notebook.Notebook.popupEnable], [gtk.notebook.Notebook.popupDisable]$(RPAREN)
 ...
 * tablets $(LPAREN)The collection of buttons and tactile sensors often found around
 * the stylus-sensitive area$(RPAREN).
 ...
 * …
 * pad_controller \= gtk_pad_controller_new $(LPAREN)window, action_group, NULL$(RPAREN);
 * ]|
 ...
 * ╰── <child>
 * ]|
 * A GtkRadioButton with indicator $(LPAREN)see [gtk.toggle_button.ToggleButton.setMode]$(RPAREN) has a
 ...
 * |[<!-- language\="C" -->
 * void create_radio_buttons $(LPAREN)void$(RPAREN) {
 * GtkWidget *window, *radio1, *radio2, *box, *entry;
 * window \= gtk_window_new $(LPAREN)GTK_WINDOW_TOPLEVEL$(RPAREN);
 * box \= gtk_box_new $(LPAREN)GTK_ORIENTATION_VERTICAL, 2$(RPAREN);
 * gtk_box_set_homogeneous $(LPAREN)GTK_BOX $(LPAREN)box$(RPAREN), TRUE$(RPAREN);
 * // Create a radio button with a GtkEntry widget
 * radio1 \= gtk_radio_button_new $(LPAREN)NULL$(RPAREN);
 * entry \= gtk_entry_new $(LPAREN)$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)radio1$(RPAREN), entry$(RPAREN);
 * // Create a radio button with a label
 * radio2 \= gtk_radio_button_new_with_label_from_widget $(LPAREN)GTK_RADIO_BUTTON $(LPAREN)radio1$(RPAREN),
 * "I’m the second radio button."$(RPAREN);
 * // Pack them into a box, then show all the widgets
 * gtk_box_pack_start $(LPAREN)GTK_BOX $(LPAREN)box$(RPAREN), radio1$(RPAREN);
 * gtk_box_pack_start $(LPAREN)GTK_BOX $(LPAREN)box$(RPAREN), radio2$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)window$(RPAREN), box$(RPAREN);
 * gtk_widget_show_all $(LPAREN)window$(RPAREN);
 * return;
 * }
 * ]|
 ...
 * $(LPAREN)with [gtk.recent_filter.RecentFilter.addPattern]$(RPAREN), on their mime type $(LPAREN)with
 * [gtk.file_filter.FileFilter.addMimeType]$(RPAREN), on the application that has
 * registered them $(LPAREN)with [gtk.recent_filter.RecentFilter.addApplication]$(RPAREN), or by
 * a custom filter function $(LPAREN)with [gtk.recent_filter.RecentFilter.addCustom]$(RPAREN).
 ...
 * 'fine-tuning' mode.
 * If the scale has an origin $(LPAREN)see [gtk.scale.Scale.setHasOrigin]$(RPAREN), there is a
 ...
 * If the scale is displaying the value $(LPAREN)see #GtkScale:draw-value$(RPAREN), there is
 ...
 * asks to move by a step $(LPAREN)using e.g. the cursor arrow keys or, if present, the
 * stepper buttons$(RPAREN) or by a page $(LPAREN)using e.g. the Page Down/Up keys$(RPAREN).
 * # CSS nodes
 * |[<!-- language\="plain" -->
 * scrollbar[.fine-tune]
 * ╰── contents
 ...
 * include the positional classes $(LPAREN).left, .right, .top, .bottom$(RPAREN) and style
 * classes related to overlay scrolling $(LPAREN).overlay-indicator, .dragging, .hovering$(RPAREN).
 */
struct GtkScrollbar
{
  GtkRange range;
 ...
 * as widgets that grow $(LPAREN)such as ellipsizing text$(RPAREN) need no such considerations.
 ...
 * |[<!-- language\="C" -->
 * GtkWidget *socket \= gtk_socket_new $(LPAREN)$(RPAREN);
 * gtk_widget_show $(LPAREN)socket$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)parent$(RPAREN), socket$(RPAREN);
 * // The following call is only necessary if one of
 * // the ancestors of the socket is not yet visible.
 * gtk_widget_realize $(LPAREN)socket$(RPAREN);
 * g_print $(LPAREN)"The ID of the sockets window is %#x\n",
 * gtk_socket_get_id $(LPAREN)socket$(RPAREN)$(RPAREN);
 * ]|
 ...
 * the style classes .up and .down. The GtkEntry subnodes $(LPAREN)if present$(RPAREN) are put
 ...
 * window \= gtk_window_new $(LPAREN)GTK_WINDOW_TOPLEVEL$(RPAREN);
 * gtk_container_set_border_width $(LPAREN)GTK_CONTAINER $(LPAREN)window$(RPAREN), 5$(RPAREN);
 * // creates the spinbutton, with no decimal places
 * button \= gtk_spin_button_new $(LPAREN)adjustment, 1.0, 0$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)window$(RPAREN), button$(RPAREN);
 * gtk_widget_show_all $(LPAREN)window$(RPAREN);
 * }
 * ]|
 ...
 * symbolic color references by the colors they refer to $(LPAREN)in a given
 * context$(RPAREN) and evaluates mix, shade and other expressions, resulting
 * in a #GdkRGBA value.
 ...
 * convenient $(LPAREN)but slightly less flexible$(RPAREN) [gtk.table.Table.attachDefaults].
...
struct GtkTextAttributes
{
   ...
   * baseline +/- ascent/descent $(LPAREN)font height$(RPAREN).
   */
  uint bgFullHeight;
 ...
 * be “anchored” $(LPAREN)inserted inline, as if they were characters$(RPAREN). The anchor
...
struct GtkTextViewClass
{
   ...
   * the drawing is done in the buffer coordinate space, but the older $(LPAREN)deprecated$(RPAREN)
 ...
 * pack a widget, $(LPAREN)such as a #GtkLabel and/or a #GtkImage$(RPAREN), into the toggle
 * button’s container. $(LPAREN)See #GtkButton for more information$(RPAREN).
 ...
 * |[<!-- language\="C" -->
 * static void output_state $(LPAREN)GtkToggleButton *source, gpointer user_data$(RPAREN) {
 * printf $(LPAREN)"Active: %d\n", gtk_toggle_button_get_active $(LPAREN)source$(RPAREN)$(RPAREN);
 * }
 * void make_toggles $(LPAREN)void$(RPAREN) {
 ...
 * gtk_toggle_button_set_mode $(LPAREN)GTK_TOGGLE_BUTTON $(LPAREN)toggle1$(RPAREN),
 * TRUE$(RPAREN);
 * g_signal_connect $(LPAREN)toggle1, "toggled",
 * G_CALLBACK $(LPAREN)output_state$(RPAREN),
 * NULL$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)box$(RPAREN), toggle1$(RPAREN);
 * text \= "Hi, I’m a toggle button.";
 * toggle2 \= gtk_toggle_button_new_with_label $(LPAREN)text$(RPAREN);
 * gtk_toggle_button_set_mode $(LPAREN)GTK_TOGGLE_BUTTON $(LPAREN)toggle2$(RPAREN),
 * FALSE$(RPAREN);
 * g_signal_connect $(LPAREN)toggle2, "toggled",
 * G_CALLBACK $(LPAREN)output_state$(RPAREN),
 * NULL$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)box$(RPAREN), toggle2$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)window$(RPAREN), box$(RPAREN);
 * gtk_widget_show_all $(LPAREN)window$(RPAREN);
 * }
 * ]|
 ...
 * GtkToolItem *item;
 * palette \= gtk_tool_palette_new $(LPAREN)$(RPAREN);
 * group \= gtk_tool_item_group_new $(LPAREN)_$(LPAREN)"Test Category"$(RPAREN)$(RPAREN);
 * gtk_container_add $(LPAREN)GTK_CONTAINER $(LPAREN)palette$(RPAREN), group$(RPAREN);
 * item \= gtk_tool_button_new $(LPAREN)NULL, _$(LPAREN)"_Open"$(RPAREN)$(RPAREN);
 * gtk_tool_button_set_icon_name $(LPAREN)GTK_TOOL_BUTTON $(LPAREN)item$(RPAREN), "document-open"$(RPAREN);
 * gtk_tool_item_group_insert $(LPAREN)GTK_TOOL_ITEM_GROUP $(LPAREN)group$(RPAREN), item, -1$(RPAREN);
 * ]|
 ...
 * |[<!-- language\="C" -->
 * static void
 * passive_canvas_drag_data_received $(LPAREN)GtkWidget        *widget,
 ...
 * palette \= gtk_widget_get_ancestor $(LPAREN)gtk_drag_get_source_widget $(LPAREN)context$(RPAREN),
 * GTK_TYPE_TOOL_PALETTE$(RPAREN);
 * if $(LPAREN)palette !\= NULL$(RPAREN)
 * item \= gtk_tool_palette_get_drag_item $(LPAREN)GTK_TOOL_PALETTE $(LPAREN)palette$(RPAREN),
 * selection$(RPAREN);
 * // Do something with item
 * }
 * GtkWidget *target, palette;
 * palette \= gtk_tool_palette_new $(LPAREN)$(RPAREN);
 * target \= gtk_drawing_area_new $(LPAREN)$(RPAREN);
 * g_signal_connect $(LPAREN)G_OBJECT $(LPAREN)target$(RPAREN), "drag-data-received",
 * G_CALLBACK $(LPAREN)passive_canvas_drag_data_received$(RPAREN), NULL$(RPAREN);
 * gtk_tool_palette_add_drag_dest $(LPAREN)GTK_TOOL_PALETTE $(LPAREN)palette$(RPAREN), target,
 * GTK_DEST_DEFAULT_ALL,
 * GTK_TOOL_PALETTE_DRAG_ITEMS,
 * GDK_ACTION_COPY$(RPAREN);
 * ]|
 * # CSS nodes
 ...
 * nodes $(LPAREN)see reference couting rule number 3 in the #GtkTreeModel section$(RPAREN).
...
struct GtkTreeSelectionClass
{
  ...
  /**
   * Signal emitted whenever the selection has $(LPAREN)possibly$(RPAREN) changed.
   */
...
/**
 * A #GtkUIManager constructs a user interface $(LPAREN)menus and toolbars$(RPAREN) from
 ...
 * |[
 * <!ELEMENT ui          $(LPAREN)menubar|toolbar|popup|accelerator$(RPAREN)* >
 * <!ELEMENT menubar     $(LPAREN)menuitem|separator|placeholder|menu$(RPAREN)* >
 * <!ELEMENT menu        $(LPAREN)menuitem|separator|placeholder|menu$(RPAREN)* >
 * <!ELEMENT popup       $(LPAREN)menuitem|separator|placeholder|menu$(RPAREN)* >
 * <!ELEMENT toolbar     $(LPAREN)toolitem|separator|placeholder$(RPAREN)* >
 * <!ELEMENT placeholder $(LPAREN)menuitem|toolitem|separator|placeholder|menu$(RPAREN)* >
 * <!ELEMENT menuitem     EMPTY >
 * <!ELEMENT toolitem     $(LPAREN)menu?$(RPAREN) >
 ...
 * action                    #IMPLIED
 * accelerators $(LPAREN)true|false$(RPAREN) #IMPLIED >
 * <!ATTLIST placeholder  name                      #IMPLIED
 * action                    #IMPLIED >
 * <!ATTLIST separator    name                      #IMPLIED
 * action                    #IMPLIED
 * expand       $(LPAREN)true|false$(RPAREN) #IMPLIED >
 * <!ATTLIST menu         name                      #IMPLIED
 * action                    #REQUIRED
 * position     $(LPAREN)top|bot$(RPAREN)    #IMPLIED >
 * <!ATTLIST menuitem     name                      #IMPLIED
 * action                    #REQUIRED
 * position     $(LPAREN)top|bot$(RPAREN)    #IMPLIED
 * always-show-image $(LPAREN)true|false$(RPAREN) #IMPLIED >
 ...
 * attributes must not contain “/” characters after parsing $(LPAREN)since that
 * would mess up path lookup$(RPAREN) and must be usable as XML attributes when
 ...
 * set to %GTK_ORIENTATION_VERTICAL instead when calling [gtk.box.Box.new_],
 ...
 * with a call like:
 * |[<!-- language\="C" -->
 * gtk_orientable_set_orientation $(LPAREN)GTK_ORIENTABLE $(LPAREN)object$(RPAREN),
 * GTK_ORIENTATION_VERTICAL$(RPAREN);
 * ]|
 * If you have a grid-like layout composed of nested boxes, and you don’t
 * need first-child or last-child styling, the recommendation is to switch
 * to #GtkGrid. For more information about migrating to #GtkGrid, see
...
struct GtkWidgetClass
{
   ...
   * [gtk.widget.Widget.setVisible]$(RPAREN) and all its parents up to the toplevel
   ...
   * GTK+ grab $(LPAREN)not a pointer or keyboard grab$(RPAREN) on another widget, or
   ...
   * #GtkWidgetClass.get_preferred_width$(LPAREN)$(RPAREN) called and then
   * #GtkWidgetClass.get_preferred_height_for_width$(LPAREN)$(RPAREN).
   ...
   * default return.
   * It’s important to note $(LPAREN)as described below$(RPAREN) that any widget
   * which trades height-for-width or width-for-height must respond properly
   * to both of the virtual methods #GtkWidgetClass.get_preferred_height_for_width$(LPAREN)$(RPAREN)
   * and #GtkWidgetClass.get_preferred_width_for_height$(LPAREN)$(RPAREN) since it might be
   ...
   * two virtual methods $(LPAREN)#GtkWidgetClass.get_preferred_width$(LPAREN)$(RPAREN) and
   * #GtkWidgetClass.get_preferred_height$(LPAREN)$(RPAREN)$(RPAREN).
   */
  extern(C) void function(GtkWidget* widget, int* minimumHeight, int* naturalHeight) getPreferredHeight;

  /**
   * This is analogous to
   * #GtkWidgetClass.get_preferred_height_for_width$(LPAREN)$(RPAREN) except that it
  ...
  /**
   * This is similar to
   * #GtkWidgetClass.get_preferred_height$(LPAREN)$(RPAREN) except that it is passed a
   ...
   * $(LPAREN)e.g. "key-press-event"$(RPAREN) and finally a generic "event-after"
   * signal.
   */
 ...
 * {
 * GtkWidgetPath *path;
 * path \= gtk_widget_path_new $(LPAREN)$(RPAREN);
 * gtk_widget_path_append_type $(LPAREN)path, GTK_TYPE_WINDOW$(RPAREN);
 * gtk_widget_path_append_type $(LPAREN)path, GTK_TYPE_BUTTON$(RPAREN);
 * }
 * ]|
 * Although more complex information, such as widget names, or
 * different classes $(LPAREN)property that may be used by other widget
 * types$(RPAREN) and intermediate regions may be included:
 ...
 * guint pos;
 * path \= gtk_widget_path_new $(LPAREN)$(RPAREN);
 * pos \= gtk_widget_path_append_type $(LPAREN)path, GTK_TYPE_NOTEBOOK$(RPAREN);
 * gtk_widget_path_iter_add_region $(LPAREN)path, pos, "tab", GTK_REGION_EVEN | GTK_REGION_FIRST$(RPAREN);
 * pos \= gtk_widget_path_append_type $(LPAREN)path, GTK_TYPE_LABEL$(RPAREN);
 * gtk_widget_path_iter_set_name $(LPAREN)path, pos, "first tab label"$(RPAREN);
 * }
 * ]|
 ...
 * your window $(LPAREN)synonymous with [gtk.window.Window.addAccelGroup].
