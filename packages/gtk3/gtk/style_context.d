...
class StyleContext : gobject.object.ObjectG
{
   ...
   * following rules:
   * |[ <!-- language\="CSS" -->
   * treeview row:nth-child$(LPAREN)even$(RPAREN) { ... }
   * treeview row:nth-child$(LPAREN)odd$(RPAREN) { ... }
   * ]|
   ...
   * See gtk_style_property_get$(LPAREN)$(RPAREN) and #GTK_STYLE_PROPERTY_MARGIN
   * for details.
   * Params:
   *   state = state to retrieve the border for
   ...
   * rendered on by the gtk_render_*$(LPAREN)$(RPAREN) functions.
   ...
   * the prelight state:
   * |[ <!-- language\="C" -->
   * gtk_style_context_notify_state_change $(LPAREN)context,
   * gtk_widget_get_window $(LPAREN)widget$(RPAREN),
   * NULL,
   * GTK_STATE_PRELIGHT,
   * button->in_button$(RPAREN);
   * ]|
  ...
  /**
   * Pushes an animatable region, so all further gtk_render_*$(LPAREN)$(RPAREN) calls between
  ...
  /**
   * Sets the sides where rendered elements $(LPAREN)mostly through
   * [gtk.global.renderFrame]$(RPAREN) will visually connect with other visual elements.
   ...
   * current region $(LPAREN)see [gtk.style_context.StyleContext.pushAnimatableRegion]$(RPAREN).
   ...
   *   styleContext = the instance the signal is connected to
   */
  alias ChangedCallbackDlg = void delegate(gtk.style_context.StyleContext styleContext);
  alias ChangedCallbackFunc = void function(gtk.style_context.StyleContext styleContext);
  ...
  if (is(T : ChangedCallbackDlg) || is(T : ChangedCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto styleContext = getVal!(gtk.style_context.StyleContext)(_paramVals);
      _dClosure.dlg(styleContext);
    }
