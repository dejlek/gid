...
class WidgetPath : gobject.boxed.Boxed
{
   ...
   * sibling matches in selectors and apply :nth-child$(LPAREN)$(RPAREN) pseudo classes.
   ...
   * ## Setting a flag
   * |[<!-- language\="C" -->
   * gtk_widget_path_iter_set_state $(LPAREN)path, pos, gtk_widget_path_iter_get_state $(LPAREN)path, pos$(RPAREN) | flag$(RPAREN);
   * ]|
   * ## Unsetting a flag
   * |[<!-- language\="C" -->
   * gtk_widget_path_iter_set_state $(LPAREN)path, pos, gtk_widget_path_iter_get_state $(LPAREN)path, pos$(RPAREN) & ~flag$(RPAREN);
   ...
   * the CSS style as closely as possible $(LPAREN)Note that there might be paths
   * that cannot be represented in CSS$(RPAREN).
