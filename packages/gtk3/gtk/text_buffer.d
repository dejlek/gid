...
class TextBuffer : gobject.object.ObjectG
{
   ...
   * [start,end$(RPAREN). start and end are revalidated to point to
   ...
   * the editability of the selection will be considered $(LPAREN)users can’t delete
   * uneditable text$(RPAREN).
   * Params:
   *   interactive = whether the deletion is caused by user interaction
   *   defaultEditable = whether the buffer is editable by default
   ...
   * The ability of creating new $(LPAREN)arbitrary!$(RPAREN) tags in the receiving buffer
   ...
   * of the selection in start and end $(LPAREN)if the selection has length 0,
   * then start and end are filled in with the same value$(RPAREN).
  ...
  /**
   * Returns the text in the range [start,end$(RPAREN). Excludes undisplayed
   * text $(LPAREN)text marked with tags that set the invisibility attribute$(RPAREN) if
  ...
  /**
   * Copies text, tags, and pixbufs between start and end $(LPAREN)the order
   * of start and end doesn’t matter$(RPAREN) and inserts the copy at iter.
   ...
   * gtk_text_buffer_set_modified $(LPAREN)buffer, FALSE$(RPAREN). When the buffer is modified,
   ...
   * invalidate the start and end iters $(LPAREN)or has to revalidate them$(RPAREN).
   * See also:
   ...
   * which run after the default handler $(LPAREN)see g_signal_connect_after$(LPAREN)$(RPAREN)$(RPAREN)
   ...
   * not invalidate the location iter $(LPAREN)or has to revalidate it$(RPAREN).
   ...
   * Note that if your handler runs before the default handler it must not
   * invalidate the location iter $(LPAREN)or has to revalidate it$(RPAREN).
   * The default signal handler revalidates it to be placed after the
   * inserted pixbuf.
   ...
   *   textBuffer = the instance the signal is connected to
   */
  alias InsertPixbufCallbackDlg = void delegate(gtk.text_iter.TextIter location, gdkpixbuf.pixbuf.Pixbuf pixbuf, gtk.text_buffer.TextBuffer textBuffer);
  alias InsertPixbufCallbackFunc = void function(gtk.text_iter.TextIter location, gdkpixbuf.pixbuf.Pixbuf pixbuf, gtk.text_buffer.TextBuffer textBuffer);
  ...
  if (is(T : InsertPixbufCallbackDlg) || is(T : InsertPixbufCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      assert(_nParams == 3, "Unexpected number of signal parameters");
      ...
      auto textBuffer = getVal!(gtk.text_buffer.TextBuffer)(_paramVals);
      auto location = getVal!(gtk.text_iter.TextIter)(&_paramVals[1]);
      auto pixbuf = getVal!(gdkpixbuf.pixbuf.Pixbuf)(&_paramVals[2]);
      _dClosure.dlg(location, pixbuf, textBuffer);
    }
   ...
   *   textBuffer = the instance the signal is connected to
   */
  alias PasteDoneCallbackDlg = void delegate(gtk.clipboard.Clipboard clipboard, gtk.text_buffer.TextBuffer textBuffer);
  alias PasteDoneCallbackFunc = void function(gtk.clipboard.Clipboard clipboard, gtk.text_buffer.TextBuffer textBuffer);
  ...
  if (is(T : PasteDoneCallbackDlg) || is(T : PasteDoneCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto _dClosure = cast(DGClosure!T*)_closure;
      auto textBuffer = getVal!(gtk.text_buffer.TextBuffer)(_paramVals);
      auto clipboard = getVal!(gtk.clipboard.Clipboard)(&_paramVals[1]);
