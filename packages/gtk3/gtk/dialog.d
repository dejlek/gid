...
class Dialog : gtk.window.Window
{
   ...
   * |[<!-- language\="C" -->
   * GtkWidget *dialog \= gtk_dialog_new $(LPAREN)$(RPAREN);
   * // Set up dialog...
   * int result \= gtk_dialog_run $(LPAREN)GTK_DIALOG $(LPAREN)dialog$(RPAREN)$(RPAREN);
   * switch $(LPAREN)result$(RPAREN)
   * {
   * case GTK_RESPONSE_ACCEPT:
   * // do_application_specific_something $(LPAREN)$(RPAREN);
   * break;
   * default:
   * // do_nothing_since_dialog_was_cancelled $(LPAREN)$(RPAREN);
   * break;
   * }
   * gtk_widget_destroy $(LPAREN)dialog$(RPAREN);
   * ]|
   * Note that even though the recursive main loop gives the effect of a
   * modal dialog $(LPAREN)it prevents the user from interacting with other
   * windows in the same window group while the dialog is run$(RPAREN), callbacks
