...
template CompletionProviderT()
{
   ...
   * with the text of proposal $(LPAREN)see [gtksource.completion_proposal.CompletionProposal.getText]$(RPAREN).
   * Here is how the default activation selects the boundaries of the word to
   * replace. The end of the word is iter. For the start of the word, it depends
   * on whether a start iter is defined for proposal $(LPAREN)see
   * [gtksource.completion_provider.CompletionProvider.getStartIter]$(RPAREN). If a start iter is defined,
   * the start of the word is the start iter. Else, the word $(LPAREN)as long as possible$(RPAREN)
   ...
   * display to the user. For example: _$(LPAREN)"Document word completion provider"$(RPAREN). The
