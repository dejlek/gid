...
class Buffer : gtk.text_buffer.TextBuffer
{
   ...
   * before initially setting the contents of the buffer $(LPAREN)e.g. when
   * loading a file in a text editor$(RPAREN).
   ...
   * $(LPAREN)i.e. if the last operation was an undo$(RPAREN).
  ...
  /**
   * Moves backward to the next toggle $(LPAREN)on or off$(RPAREN) of the context class. If no
  ...
  /**
   * Moves forward to the next toggle $(LPAREN)on or off$(RPAREN) of the context class. If no
   * matching context class toggles are found, returns %FALSE, otherwise %TRUE.
   * Does not return toggles located at iter, only toggles after iter. Sets
   * iter to the location of the toggle, or to the end of the buffer if no
   * toggle is found.
