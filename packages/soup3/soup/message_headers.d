...
class MessageHeaders : Boxed
{
   ...
   * $(LPAREN)classMessage does this automatically for its own headers. You would only
   ...
   * suggestion of a filename to use. $(LPAREN)If the parameter value in the
   * header contains an absolute or relative path, libsoup will truncate
   * it down to just the final path component, so you do not need to
   * test this yourself.$(RPAREN)
   ...
   *   header, %FALSE if not $(LPAREN)in which case *disposition and *params
   *   will be unchanged$(RPAREN).
   */
   ...
   * Params:
   *   params = return location for the Content-Type parameters $(LPAREN)eg, "charset"$(RPAREN), or
   *     %NULL
   * Returns: a string with the value of the
   *   "Content-Type" header or %NULL if hdrs does not contain that
   *   header or it cannot be parsed $(LPAREN)in which case *params will be
   *   unchanged$(RPAREN).
   */
   ...
   * complete response body $(LPAREN)rather than pausing the message and
   * returning some of the response body later$(RPAREN), and there is a Range
   ...
   * the range$(LPAREN)s$(RPAREN) requested by the client.
   ...
   * $(LPAREN)If name is present in hdrs, then this is equivalent to calling
   * funcheader_contains on its value.$(RPAREN)
   * Params:
   *   name = header name
   ...
   * $(LPAREN)case-insensitively$(RPAREN) equal to value.
   * Params:
   *   name = header name
   ...
   * $(LPAREN)Note that total_length is the total length of the entire resource
   ...
   * large message body $(LPAREN)eg, via POST or PUT$(RPAREN), and want to give the
   * server a chance to reject the request after seeing just the headers
   * $(LPAREN)eg, because it will require authentication before allowing you to
   * post, or because you're POSTing to a URL that doesn't exist$(RPAREN). This
