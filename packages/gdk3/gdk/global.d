 ...
 * $(LPAREN)source_type \=\= #GL_RENDERBUFFER$(RPAREN) or a texture id $(LPAREN)source_type \=\= #GL_TEXTURE$(RPAREN)
 * and draws it onto cr with an OVER operation, respecting the current clip.
 * The top left corner of the rectangle specified by x, y, width and height
 * will be drawn at the current $(LPAREN)0,0$(RPAREN) position of the cairo_t.
 ...
 * Params:
 *   cr = a cairo context
 *   window = The window we're rendering for $(LPAREN)not necessarily into$(RPAREN)
 ...
 *     gdk_drag_find_window$(LPAREN)$(RPAREN)
 *   protocol = the DND protocol in use, obtained by gdk_drag_find_window$(LPAREN)$(RPAREN)
 *   xRoot = the x position of the pointer in root coordinates
 *   yRoot = the y position of the pointer in root coordinates
 ...
 * You can use gdk_x11_display_error_trap_push$(LPAREN)$(RPAREN) to ignore errors
 * on only a single display.
 * ## Trapping an X error
 * |[<!-- language\="C" -->
 * gdk_error_trap_push $(LPAREN)$(RPAREN);
 * // ... Call the X function which may cause an error here ...
 * if $(LPAREN)gdk_error_trap_pop $(LPAREN)$(RPAREN)$(RPAREN)
 * {
 ...
 * $(LPAREN)as in a straight line going from event1 to event2$(RPAREN) will be returned.
 * Params:
 *   event1 = first #GdkEvent
 *   event2 = second #GdkEvent
...
/**
 * Obtains the root window $(LPAREN)parent all other windows are inside$(RPAREN)
 ...
 * Deprecated: Call gdk_display_get_name $(LPAREN)gdk_display_get_default $(LPAREN)$(RPAREN)$(RPAREN)$(RPAREN)
 *   instead.
 */
string getDisplay()
{
 ...
 * commandline option, the default value is the program name $(LPAREN)determined
 * with [glib.global.getPrgname]$(RPAREN) with the first character converted to uppercase.
...
/**
 * Convert from a GDK key symbol to the corresponding ISO10646 $(LPAREN)Unicode$(RPAREN)
 * character.
 * Params:
 ...
 * $(LPAREN)See [gdk.screen.Screen.listVisuals]$(RPAREN)
 ...
 * Deprecated: Use gdk_screen_list_visuals $(LPAREN)gdk_screen_get_default $(LPAREN)$(RPAREN)$(RPAREN).
 */
 ...
 * The newly created context will have the default font options $(LPAREN)see
 * #cairo_font_options_t$(RPAREN) for the default screen; if these options
 ...
 * $(LPAREN)see #cairo_font_options_t$(RPAREN) for the display; if these options
 * change it will not be updated. Using [gtk.widget.Widget.getPangoContext]
 * is more convenient if you want to keep a context around and track
 ...
 * the widget you intend to render text onto.
 * The newly created context will have the default font options
 * $(LPAREN)see #cairo_font_options_t$(RPAREN) for the screen; if these options
 * change it will not be updated. Using [gtk.widget.Widget.getPangoContext]
 * is more convenient if you want to keep a context around and track
 * changes to the screen’s font rendering settings.
 * Params:
...
/**
 * Transfers image data from a #cairo_surface_t and converts it to an RGB$(LPAREN)A$(RPAREN)
...
/**
 * Transfers image data from a #GdkWindow and converts it to an RGB$(LPAREN)A$(RPAREN)
 * representation inside a #GdkPixbuf.
 * In other words, copies image data from a server-side drawable to a
 * client-side RGB$(LPAREN)A$(RPAREN) buffer. This allows you to efficiently read
 ...
 * If the window is not mapped $(LPAREN)typically because it’s iconified/minimized
 * or not on the current workspace$(RPAREN), then %NULL will be returned.
...
/**
 * Grabs the pointer $(LPAREN)usually a mouse$(RPAREN) so that all events are passed to this
 ...
 *     owner_events. Note that only pointer events $(LPAREN)i.e. button and motion events$(RPAREN)
 *     may be selected.
 ...
 * The XGetWindowProperty$(LPAREN)$(RPAREN) function that [gdk.global.propertyGet]
 * uses has a very confusing and complicated set of semantics.
 * Unfortunately, [gdk.global.propertyGet] makes the situation
 * worse instead of better $(LPAREN)the semantics should be considered
 * undefined$(RPAREN), and also prints warnings to stderr in cases where it
 * should return a useful error to the program. You are advised to use
 * XGetWindowProperty$(LPAREN)$(RPAREN) directly until a replacement function for
 ...
 *     $(LPAREN)so be careful not to pass a value that might overflow
 *     when rounded up$(RPAREN).
 ...
 * $(LPAREN)[gdk.global.listVisuals]$(RPAREN) and then looking at the depth field in each
 ...
 * screen. It’s equivalent to listing the visuals
 * $(LPAREN)[gdk.global.listVisuals]$(RPAREN) and then looking at the type field in each
 * visual, removing duplicates.
 * The array returned by this function should not be freed.
 * Params:
 *   visualTypes = return
 ...
 * For example,
 * |[<!-- language\="C" -->
 * gdk_set_allowed_backends $(LPAREN)"wayland,quartz,*"$(RPAREN);
 * ]|
 ...
 * Note that GTK+ must be compiled with debugging $(LPAREN)that is,
 * configured using the `--enable-debug` option$(RPAREN)
 * to use this option.
 * Params:
 ...
 * It will warp the mouse pointer to the given $(LPAREN)x,y$(RPAREN) coordinates
 * within window and simulate a button press or release event.
 * Because the mouse pointer needs to be warped to the target
 * location, use of this function outside of test programs that
 * run in their own virtual windowing system $(LPAREN)e.g. Xvfb$(RPAREN) is not
 * recommended.
...
/**
 * This function is intended to be used in GTK+ test programs.
 * If $(LPAREN)x,y$(RPAREN) are > $(LPAREN)-1,-1$(RPAREN), it will warp the mouse pointer to
 * the given $(LPAREN)x,y$(RPAREN) coordinates within window and simulate a
 * key press or release event.
 * When the mouse pointer is warped to the target location, use
 * of this function outside of test programs that run in their
 * own virtual windowing system $(LPAREN)e.g. Xvfb$(RPAREN) is not recommended.
 * If $(LPAREN)x,y$(RPAREN) are passed as $(LPAREN)-1,-1$(RPAREN), the mouse pointer will not
 ...
 * following use case, where you have to worry about idle_callback$(LPAREN)$(RPAREN)
 ...
 * {
 * // [gdk.global.threadsEnter]; would be needed for [glib.global.idleAdd]
 * SomeWidget *self \= data;
 * // do stuff with self
 * self->idle_id \= 0;
 * // [gdk.global.threadsLeave]; would be needed for [glib.global.idleAdd]
 * return FALSE;
 * }
 * static void
 * some_widget_do_stuff_later $(LPAREN)SomeWidget *self$(RPAREN)
 * {
 * self->idle_id \= gdk_threads_add_idle $(LPAREN)idle_callback, self$(RPAREN)
 * // using [glib.global.idleAdd] here would require thread protection in the callback
 * }
 * static void
 ...
 * |[<!-- language\="C" -->
 * static gboolean timeout_callback $(LPAREN)gpointer data$(RPAREN)
 * {
 * SomeWidget *self \= data;
 * // do stuff with self
 * self->timeout_id \= 0;
 ...
 * }
 * static void some_widget_do_stuff_later $(LPAREN)SomeWidget *self$(RPAREN)
 * {
 * self->timeout_id \= g_timeout_add $(LPAREN)timeout_callback, self$(RPAREN)
 * }
 * static void some_widget_finalize $(LPAREN)GObject *object$(RPAREN)
 * {
 * SomeWidget *self \= SOME_WIDGET $(LPAREN)object$(RPAREN);
 * if $(LPAREN)self->timeout_id$(RPAREN)
 * g_source_remove $(LPAREN)self->timeout_id$(RPAREN);
 * G_OBJECT_CLASS $(LPAREN)parent_class$(RPAREN)->finalize $(LPAREN)object$(RPAREN);
 * }
 * ]|
 * Params:
 ...
 *   conversion failed. $(LPAREN)It should not fail for any properly
 *   formed UTF-8 string unless system limits like memory or
 *   file descriptors are exceeded.$(RPAREN)
 */
