...
class Window : gobject.object.ObjectG
{
   ...
   * window under the mouse pointer is not known to GDK $(LPAREN)if the window
   * belongs to another application and a #GdkWindow hasn’t been created
   * for it with gdk_window_foreign_new$(LPAREN)$(RPAREN)$(RPAREN)
   ...
   * set of geometry hints $(LPAREN)such as minimum and maximum size$(RPAREN).
   * Params:
   ...
   * implementation, a backing store $(LPAREN)offscreen buffer$(RPAREN) large enough to
   * contain region will be created. The backing store will be initialized
   * with the background color or background surface for window. Then, all
   * drawing operations performed on window will be diverted to the
   * backing store. When you call gdk_window_end_frame$(LPAREN)$(RPAREN), the contents of
  ...
  /**
   * Begins a window move operation $(LPAREN)for a toplevel window$(RPAREN).
  ...
  /**
   * Indicates that you are beginning the process of redrawing region.
   * A backing store $(LPAREN)offscreen buffer$(RPAREN) large enough to contain region
   ...
   * [gdk.window.Window.beginPaintRegion]. $(LPAREN)You can disable the automatic
   * calls around expose events on a widget-by-widget basis by calling
   * [gtk.widget.Widget.setDoubleBuffered].$(RPAREN)
  ...
  /**
   * Begins a window resize operation $(LPAREN)for a toplevel window$(RPAREN).
   * This function assumes that the drag is controlled by the
   ...
   *   rootX = root window X coordinate of mouse click that began the drag
   *   rootY = root window Y coordinate of mouse click that began the drag
   *   timestamp = timestamp of mouse click that began the drag $(LPAREN)use [gdk.event.Event.getTime]$(RPAREN)
   */
   ...
   * For offscreen windows however $(LPAREN)which can be arbitrarily transformed$(RPAREN),
   ...
   * use:
   * |[<!-- language\="C" -->
   * int scale \= gdk_window_get_scale_factor $(LPAREN)window$(RPAREN);
   * int width \= gdk_window_get_width $(LPAREN)window$(RPAREN) * scale;
   * int height \= gdk_window_get_height $(LPAREN)window$(RPAREN) * scale;
   * // format is set elsewhere
   * cairo_surface_t *surface \=
   * gdk_window_create_similar_image_surface $(LPAREN)window,
   * format,
   * width, height,
   * scale$(RPAREN);
   * ]|
  ...
  /**
   * Attempt to deiconify $(LPAREN)unminimize$(RPAREN) window. On X11 the window manager may
   ...
   * gdk_window_begin_frame$(LPAREN)$(RPAREN) has been completed.
   * This function will take care of destroying the #GdkDrawingContext.
   * It is an error to call this function without a matching
   * gdk_window_begin_frame$(LPAREN)$(RPAREN) first.
   * Params:
   ...
   * The returned list must be freed, but the elements in the
   * list need not be.
   * The list is returned in $(LPAREN)relative$(RPAREN) stacking order, i.e. the
   ...
   *   $(LPAREN)as with [gdk.device.Device.getWindowAtPosition]$(RPAREN), or %NULL if the
   ...
   * coordinates. To get the position of the window itself $(LPAREN)rather than
   * the frame$(RPAREN) in root window coordinates, use [gdk.window.Window.getOrigin].
   * Params:
   ...
   * window decorations $(LPAREN)titlebar, etc.$(RPAREN) rather than relative to the
   * root window $(LPAREN)screen-size background window$(RPAREN).
   ...
   * Params:
   *   x = return location for X coordinate of window $(LPAREN)relative to its parent$(RPAREN)
   *   y = return location for Y coordinate of window $(LPAREN)relative to its parent$(RPAREN)
   ...
   * $(LPAREN)Compare with [gdk.window.Window.getPosition] and
   * [gdk.window.Window.getGeometry] which return the position of a window
   * relative to its parent window.$(RPAREN)
   * Params:
   ...
   *   pointer $(LPAREN)as with [gdk.window.Window.atPointer]$(RPAREN), or %NULL if the window
  ...
  /**
   * Asks to iconify $(LPAREN)minimize$(RPAREN) window. The window manager may choose
   ...
   * mapped. $(LPAREN)This is not necessarily "viewable" in the X sense, since
  ...
  /**
   * Checks whether the window has been mapped $(LPAREN)with [gdk.window.Window.show] or
   * [gdk.window.Window.showUnraised]$(RPAREN).
  ...
  /**
   * Lowers window to the bottom of the Z-order $(LPAREN)stacking order$(RPAREN), so that
  ...
  /**
   * If you call this during a paint $(LPAREN)e.g. between [gdk.window.Window.beginPaintRegion]
   ...
   * visual effects. $(LPAREN)i.e. the user may be able to see the window first
   * move, then resize, if you don’t use [gdk.window.Window.moveResize].$(RPAREN)
   * Params:
   ...
   * expose event will cover the entire update area for the window $(LPAREN)see
   * [gdk.window.Window.invalidateRegion] for details$(RPAREN). Normally GDK calls
   * [gdk.window.Window.processAllUpdates] on your behalf, so there’s no
   * need to call this function unless you want to force expose events
   * to be delivered immediately and synchronously $(LPAREN)vs. the usual
   * case, where GDK delivers them in an idle handler$(RPAREN). Occasionally
  ...
  /**
   * Raises window to the top of the Z-order $(LPAREN)stacking order$(RPAREN), so that
  ...
  /**
   * Changes the position of  window in the Z-order $(LPAREN)stacking order$(RPAREN), so that
   * it is above sibling $(LPAREN)if above is %TRUE$(RPAREN) or below sibling $(LPAREN)if above is
   * %FALSE$(RPAREN).
   * If sibling is %NULL, then this either raises $(LPAREN)if above is %TRUE$(RPAREN) or
   * lowers the window.
   * If window is a toplevel, the window manager may choose to deny the
   ...
   * window using gdk_x11_get_parent_relative_pattern$(LPAREN)$(RPAREN).
   ...
   * This call is only supported on some systems $(LPAREN)currently,
   * only X11 with new enough Xcomposite and Xdamage extensions$(RPAREN).
  ...
  /**
   * Sets the event mask for a given device $(LPAREN)Normally a floating device, not
   * attached to any visible pointer$(RPAREN) to window. For example, an event mask
   ...
   * interactively $(LPAREN)such as popups from network activity$(RPAREN).
  ...
  /**
   * Specifies whether the window should span over all monitors $(LPAREN)in a multi-head
   * setup$(RPAREN) or only the current monitor when in fullscreen mode.
   ...
   * will typically  $(LPAREN)but is not required to$(RPAREN) also constrain the
   ...
   * with opacity 0 being fully transparent and 1 fully opaque. $(LPAREN)Values
   * of the opacity parameter are clamped to the [0,1] range.$(RPAREN)
   * For toplevel windows this depends on support from the windowing system
   * that may not always be there. For instance, On X11, this works only on
   * X screens with a compositing manager running. On Wayland, there is no
   * per-window opacity value that the compositor would apply. Instead, use
   * `gdk_window_set_opaque_region $(LPAREN)window, NULL$(RPAREN)` to tell the compositor
   * that the entire window is $(LPAREN)potentially$(RPAREN) non-opaque, and draw your content
  ...
  /**
   * Toggles whether a window should appear in a pager $(LPAREN)workspace
   * switcher, or other desktop utility program that displays a small
   * thumbnail representation of the windows on the desktop$(RPAREN). If a
  ...
  /**
   * Sets the event mask for any floating device $(LPAREN)i.e. not attached to any
   * visible pointer$(RPAREN) that has the source defined as source. This event
   ...
   * $(LPAREN)using [gdk.window.Window.setIconName]$(RPAREN), the icon name will be set to
   * title as well. title must be in UTF-8 encoding $(LPAREN)as with all
   * user-readable strings in GDK/GTK+$(RPAREN). title may not be %NULL.
   * Params:
   ...
   * top of the window stack $(LPAREN)moves the window to the front of the
   * Z-order$(RPAREN).
   ...
   * XMapWindow$(LPAREN)$(RPAREN) $(LPAREN)it also updates some internal GDK state, which means
   * that you can’t really use XMapWindow$(LPAREN)$(RPAREN) directly on a GDK window$(RPAREN).
   */
  void showUnraised()
  {
   ...
   * $(LPAREN)When using #GtkWindow, [gtk.window.Window.stick] may be more useful.$(RPAREN)
  ...
  /**
   * Withdraws a window $(LPAREN)unmaps it and asks the window manager to forget about it$(RPAREN).
   ...
   * needs its surface $(LPAREN)re$(RPAREN)created, which happens either when the
   ...
   */
  alias CreateSurfaceCallbackDlg = cairo.surface.Surface delegate(int width, int height, gdk.window.Window window);
  alias CreateSurfaceCallbackFunc = cairo.surface.Surface function(int width, int height, gdk.window.Window window);
  ...
  if (is(T : CreateSurfaceCallbackDlg) || is(T : CreateSurfaceCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      assert(_nParams == 3, "Unexpected number of signal parameters");
      auto _dClosure = cast(DGClosure!T*)_closure;
      auto window = getVal!(gdk.window.Window)(_paramVals);
      auto width = getVal!(int)(&_paramVals[1]);
      auto height = getVal!(int)(&_paramVals[2]);
      auto _retval = _dClosure.dlg(width, height, window);
   ...
   *   window = the instance the signal is connected to
   */
  alias MovedToRectCallbackDlg = void delegate(void* flippedRect, void* finalRect, bool flippedX, bool flippedY, gdk.window.Window window);
  alias MovedToRectCallbackFunc = void function(void* flippedRect, void* finalRect, bool flippedX, bool flippedY, gdk.window.Window window);
  ...
  if (is(T : MovedToRectCallbackDlg) || is(T : MovedToRectCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      _dClosure.dlg(flippedRect, finalRect, flippedX, flippedY, window);
    }
   ...
   */
  alias PickEmbeddedChildCallbackDlg = gdk.window.Window delegate(double x, double y, gdk.window.Window window);
  alias PickEmbeddedChildCallbackFunc = gdk.window.Window function(double x, double y, gdk.window.Window window);
  ...
  if (is(T : PickEmbeddedChildCallbackDlg) || is(T : PickEmbeddedChildCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      assert(_nParams == 3, "Unexpected number of signal parameters");
      ...
      auto _retval = _dClosure.dlg(x, y, window);
