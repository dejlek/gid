...
/**
 * Used to represent native events $(LPAREN)XEvents for the X11
 * backend, MSGs for Win32$(RPAREN).
 */
 ...
 * axis $(LPAREN)valuator$(RPAREN) maps onto the predefined valuator
...
enum GdkAxisUse
{
  ...
  /**
   * the axis is used for pen/tablet distance information. $(LPAREN)Since: 3.22$(RPAREN)
   */
  Distance = 7,

  /**
   * the axis is used for pen rotation information. $(LPAREN)Since: 3.22$(RPAREN)
   */
  Rotation = 8,

  /**
   * the axis is used for pen slider information. $(LPAREN)Since: 3.22$(RPAREN)
   */
  Slider = 9,
...
{
  /**
   * Device is a master $(LPAREN)or virtual$(RPAREN) device. There will
   * be an associated focus indicator on the screen.
   */
  Master = 0,

  /**
   * Device is a slave $(LPAREN)or physical$(RPAREN) device.
   */
  Slave = 1,
...
enum GdkDragProtocol
{
  ...
  /**
   * The complex OLE2 DND protocol $(LPAREN)not implemented$(RPAREN).
   */
  Ole2 = 5,
 ...
 * be sent, some of which are marked as a hint $(LPAREN)the is_hint member is
 * %TRUE$(RPAREN). To receive more motion events after a motion hint event,
 ...
 * type %GDK_TOUCH_BEGIN and %GDK_TOUCH_END $(LPAREN)or %GDK_TOUCH_CANCEL$(RPAREN).
 ...
 * invalid $(LPAREN)eg `Gdk.EventType.2ButtonPress`, where a
 * symbol is not allowed to start with a number$(RPAREN). In that case, the
...
enum GdkEventType
{
  ...
  /**
   * a mouse button has been double-clicked $(LPAREN)clicked twice
   * within a short period of time$(RPAREN). Note that each click also generates a
...
/**
 * Indicates which monitor $(LPAREN)in a multi-head setup$(RPAREN) a window should span over
 ...
 * invoking menu shortcuts $(LPAREN)accelerators$(RPAREN), whereas on Apple computers
 * it’s the Command key $(LPAREN)which correspond to %GDK_CONTROL_MASK and
 * %GDK_MOD2_MASK, respectively$(RPAREN).
 */
enum GdkModifierIntent
{
   ...
   * groups $(LPAREN)AltGr on X11/Windows and Option/Alt on OS X$(RPAREN).
   */
  ShiftGroup = 5,
...
enum GdkModifierType : uint
{
  ...
  /**
   * the fifth modifier key $(LPAREN)it depends on the modifier
   * mapping of the X server which key is interpreted as this modifier$(RPAREN).
   */
  Mod2Mask = 16,

  /**
   * the sixth modifier key $(LPAREN)it depends on the modifier
   * mapping of the X server which key is interpreted as this modifier$(RPAREN).
   */
  Mod3Mask = 32,

  /**
   * the seventh modifier key $(LPAREN)it depends on the modifier
   * mapping of the X server which key is interpreted as this modifier$(RPAREN).
   */
  Mod4Mask = 64,

  /**
   * the eighth modifier key $(LPAREN)it depends on the modifier
   * mapping of the X server which key is interpreted as this modifier$(RPAREN).
   */
  Mod5Mask = 128,
   ...
   * between $(LPAREN)keyval, modifiers$(RPAREN) pairs from key press and release events.
   */
...
enum GdkWindowState : uint
{
  ...
  /**
   * the window is presented as focused $(LPAREN)with active decorations$(RPAREN).
   */
  Focused = 128,
...
enum GdkWindowType
{
  ...
  /**
   * toplevel window $(LPAREN)used to implement #GtkWindow$(RPAREN)
   */
  Toplevel = 1,

  /**
   * child window $(LPAREN)used to implement e.g. #GtkEntry$(RPAREN)
   */
  Child = 2,

  /**
   * override redirect temporary window $(LPAREN)used to implement
   * #GtkMenu$(RPAREN)
   */
  Temp = 3,

  /**
   * foreign window $(LPAREN)see gdk_window_foreign_new$(LPAREN)$(RPAREN)$(RPAREN)
   */
  Foreign = 4,

  /**
   * offscreen window $(LPAREN)see
   * [Offscreen Windows][OFFSCREEN-WINDOWS]$(RPAREN). Since 2.18
   */
  Offscreen = 5,
 ...
 * gdk_app_launch_context_set_screen $(LPAREN)screen$(RPAREN);
 * gdk_app_launch_context_set_timestamp $(LPAREN)event->time$(RPAREN);
 ...
 * effective $(LPAREN)current$(RPAREN) for each given group, different groups may have
 ...
 * if $(LPAREN)GDK_IS_QUARTZ_DISPLAY $(LPAREN)display$(RPAREN)$(RPAREN)
 * {
...
{
  /**
   * the type of the event $(LPAREN)%GDK_BUTTON_PRESS, %GDK_2BUTTON_PRESS,
   * %GDK_3BUTTON_PRESS or %GDK_BUTTON_RELEASE$(RPAREN).
   */
  GdkEventType type;
   ...
   * the modifier keys $(LPAREN)e.g. Control, Shift and Alt$(RPAREN) and the pointer
...
{
  /**
   * the type of the event $(LPAREN)%GDK_CONFIGURE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_ENTER_NOTIFY or %GDK_LEAVE_NOTIFY$(RPAREN).
   */
  GdkEventType type;
  ...
  /**
   * the crossing mode $(LPAREN)%GDK_CROSSING_NORMAL, %GDK_CROSSING_GRAB,
   * %GDK_CROSSING_UNGRAB, %GDK_CROSSING_GTK_GRAB, %GDK_CROSSING_GTK_UNGRAB or
   * %GDK_CROSSING_STATE_CHANGED$(RPAREN).  %GDK_CROSSING_GTK_GRAB, %GDK_CROSSING_GTK_UNGRAB,
  ...
  /**
   * the kind of crossing that happened $(LPAREN)%GDK_NOTIFY_INFERIOR,
   * %GDK_NOTIFY_ANCESTOR, %GDK_NOTIFY_VIRTUAL, %GDK_NOTIFY_NONLINEAR or
   * %GDK_NOTIFY_NONLINEAR_VIRTUAL$(RPAREN).
   */
...
{
  /**
   * the type of the event $(LPAREN)%GDK_DRAG_ENTER, %GDK_DRAG_LEAVE,
   * %GDK_DRAG_MOTION, %GDK_DRAG_STATUS, %GDK_DROP_START or
   * %GDK_DROP_FINISHED$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_EXPOSE or %GDK_DAMAGE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_FOCUS_CHANGE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_GRAB_BROKEN$(RPAREN)
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_KEY_PRESS or %GDK_KEY_RELEASE$(RPAREN).
   */
  GdkEventType type;
   ...
   * input of text is using input methods $(LPAREN)see #GtkIMContext$(RPAREN), so this
   * field is deprecated and should never be used.
   * $(LPAREN)[gdk.global.unicodeToKeyval] provides a non-deprecated way of getting
   * an approximate translation for a key.$(RPAREN) The string is encoded in the
   * encoding of the current locale $(LPAREN)Note: this for backwards compatibility:
   * strings in GTK+ and GDK are typically in UTF-8.$(RPAREN) and NUL-terminated.
...
{
  /**
   * the type of the event $(LPAREN)%GDK_OWNER_CHANGE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_PAD_RING or %GDK_PAD_STRIP$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_PAD_BUTTON_PRESS or %GDK_PAD_BUTTON_RELEASE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_PAD_GROUP_MODE$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_PROPERTY_NOTIFY$(RPAREN).
   */
  GdkEventType type;
   ...
   * $(LPAREN)%GDK_PROPERTY_NEW_VALUE$(RPAREN) or deleted $(LPAREN)%GDK_PROPERTY_DELETE$(RPAREN).
   */
...
{
  /**
   * the type of the event $(LPAREN)%GDK_PROXIMITY_IN or %GDK_PROXIMITY_OUT$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_SCROLL$(RPAREN).
   */
  GdkEventType type;
  ...
  /**
   * the direction to scroll to $(LPAREN)one of %GDK_SCROLL_UP,
   * %GDK_SCROLL_DOWN, %GDK_SCROLL_LEFT, %GDK_SCROLL_RIGHT or
   * %GDK_SCROLL_SMOOTH$(RPAREN).
   */
...
{
  /**
   * the type of the event $(LPAREN)%GDK_SELECTION_CLEAR,
   * %GDK_SELECTION_NOTIFY or %GDK_SELECTION_REQUEST$(RPAREN).
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_SETTING$(RPAREN).
   */
  GdkEventType type;
  ...
  /**
   * what happened to the setting $(LPAREN)%GDK_SETTING_ACTION_NEW,
   * %GDK_SETTING_ACTION_CHANGED or %GDK_SETTING_ACTION_DELETED$(RPAREN).
   */
...
{
  /**
   * the type of the event $(LPAREN)%GDK_TOUCH_BEGIN, %GDK_TOUCH_UPDATE,
   * %GDK_TOUCH_END, %GDK_TOUCH_CANCEL$(RPAREN)
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_TOUCHPAD_PINCH$(RPAREN)
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_TOUCHPAD_SWIPE$(RPAREN)
   */
  GdkEventType type;
...
{
  /**
   * the type of the event $(LPAREN)%GDK_VISIBILITY_NOTIFY$(RPAREN).
   */
  GdkEventType type;
  ...
  /**
   * the new visibility state $(LPAREN)%GDK_VISIBILITY_FULLY_OBSCURED,
   * %GDK_VISIBILITY_PARTIAL or %GDK_VISIBILITY_UNOBSCURED$(RPAREN).
   */
...
{
  /**
   * the type of the event $(LPAREN)%GDK_WINDOW_STATE$(RPAREN).
   */
  GdkEventType type;
 ...
 * gtk_window_set_geometry_hints $(LPAREN)GTK_WINDOW $(LPAREN)toplevel$(RPAREN),
 * GTK_WIDGET $(LPAREN)terminal$(RPAREN),
 * &hints,
 * GDK_HINT_RESIZE_INC |
 * GDK_HINT_MIN_SIZE |
 * GDK_HINT_BASE_SIZE$(RPAREN);
 * ]|
...
{
  /**
   * minimum width of window $(LPAREN)or -1 to use requisition, with
   * #GtkWindow only$(RPAREN)
   */
  int minWidth;

  /**
   * minimum height of window $(LPAREN)or -1 to use requisition, with
   * #GtkWindow only$(RPAREN)
   */
  int minHeight;

  /**
   * maximum width of window $(LPAREN)or -1 to use requisition, with
   * #GtkWindow only$(RPAREN)
   */
  int maxWidth;

  /**
   * maximum height of window $(LPAREN)or -1 to use requisition, with
   * #GtkWindow only$(RPAREN)
   */
  int maxHeight;
   ...
   * is any integer $(LPAREN)-1 allowed with #GtkWindow$(RPAREN)
   */
  int baseWidth;

  /**
   * allowed window widths are @base_height + @height_inc * N where
   * N is any integer $(LPAREN)-1 allowed with #GtkWindow$(RPAREN)
   */
  int baseHeight;
 ...
 * $(LPAREN)including a hardware key, a modifier mask, and active keyboard group$(RPAREN)
...
{
  /**
   * title of the window $(LPAREN)for toplevel windows$(RPAREN)
   */
  char* title;

  /**
   * event mask $(LPAREN)see [gdk.window.Window.setEvents]$(RPAREN)
   */
  int eventMask;

  /**
   * X coordinate relative to parent window $(LPAREN)see [gdk.window.Window.move]$(RPAREN)
   */
  int x;

  /**
   * Y coordinate relative to parent window $(LPAREN)see [gdk.window.Window.move]$(RPAREN)
   */
  int y;
  ...
  /**
   * #GDK_INPUT_OUTPUT $(LPAREN)normal window$(RPAREN) or #GDK_INPUT_ONLY $(LPAREN)invisible
   * window that receives events$(RPAREN)
   */
  ...
  /**
   * cursor for the window $(LPAREN)see [gdk.window.Window.setCursor]$(RPAREN)
   */
  GdkCursor* cursor;

  /**
   * don’t use $(LPAREN)see [gtk.window.Window.setWmclass]$(RPAREN)
   */
  char* wmclassName;
