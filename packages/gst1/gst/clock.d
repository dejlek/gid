...
class Clock : gst.object.ObjectGst
{
  ...
  */
  ...
  /**
    ...
    ## Parameters
    $(LIST
      * $(B synced)       if the clock is synced now
      * $(B clock) the instance the signal is connected to
    )
  */
  alias SyncedCallbackDlg = void delegate(bool synced, gst.clock.Clock clock);

  /** ditto */
  alias SyncedCallbackFunc = void function(bool synced, gst.clock.Clock clock);
  ...
  */
  ...
  if (is(T : SyncedCallbackDlg) || is(T : SyncedCallbackFunc))
  {
    extern(C) void _cmarshal(GClosure* _closure, GValue* _returnValue, uint _nParams, const(GValue)* _paramVals, void* _invocHint, void* _marshalData)
    {
      ...
      auto clock = getVal!(gst.clock.Clock)(_paramVals);
      auto synced = getVal!(bool)(&_paramVals[1]);
      _dClosure.dlg(synced, clock);
    }
